...
import {
  ...
  CANDIDATE_COPY_RECEIPT_VERSION,
  exactCopyHash,
  prepareCandidateCopy,
} from './application-humanizer.mjs';
...
/**
 * Verify the artifact boundary for a generated cover letter. The manifest
 * receipt is not enough on its own: the exact text file must still match the
 * output hash recorded when the Humanizer stage passed.
 * @param {Record<string, unknown>} manifest
 * @param {string} [textPath]
 */
export function candidateCopyArtifactReadiness(manifest, textPath = '') {
  const coverLetter = manifest?.coverLetter;
  const receipt = coverLetter?.copyQuality;
  if (!receipt || receipt.schema !== CANDIDATE_COPY_RECEIPT_SCHEMA || receipt.version !== CANDIDATE_COPY_RECEIPT_VERSION) {
    return { ok: false, reason: 'cover letter has no valid candidate-copy receipt' };
  }
  if (receipt.status !== 'passed' || receipt.passed !== true || receipt.finalReady !== true
    || receipt.humanizer?.status !== 'passed' || receipt.humanizer?.passed !== true
    || receipt.style?.passed !== true) {
    return { ok: false, reason: 'cover letter Humanizer gate is not passed' };
  }
  const expectedPath = String(coverLetter.textPath || '');
  const actualPath = String(textPath || expectedPath);
  if (!actualPath || (expectedPath && actualPath !== expectedPath) || !existsSync(actualPath)) {
    return { ok: false, reason: 'cover letter text artifact is missing or does not match the receipt' };
  }
  const outputHash = exactCopyHash(readFileSync(actualPath, 'utf8'));
  if (receipt.outputHash !== outputHash) {
    return { ok: false, reason: 'cover letter text changed after the Humanizer receipt was created' };
  }
  return { ok: true, reason: 'cover letter text matches the passed Humanizer receipt', receipt };
}
...
export function assessResumeReuse(item, options = {}) {
  ...
  if (!resolved.ok || !resolved.manifest) {
    return { ...base, reasonCodes: ['no-canonical-lane-resume'], reasons: [resolved.reason || 'no configured canonical lane resume was found'] };
  }
  return {
    decision: 'lane',
    reasonCodes: ['canonical-lane-resume', 'per-job-generation-retired'],
    reasons: ['the configured canonical lane resume is required; per-job resume generation is retired'],
    manifestPath: '',
    artifactPath: resolved.artifactPath,
    coverage: { lane: resolved.request.lane, requiredKeywords: [], coveredKeywords: [], missingKeywords: [], score: 1 },
...
function buildSummary(profile, item, lane, projects, keywords) {
  ...
  return `Software engineer targeting ${role} roles, with three Amazon SDE internships and Lead Engineer client delivery experience. Builds ${focus} across ${projectText}. Available full-time now.`;
}
...
function buildCoverLetterHtml({ profile, item, lane, projects, cover }) {
  ...
  const values = {
    ...
    CREDENTIALS_BLOCK: `<div class="credentials">3x Amazon SDE intern | Lead Engineer, Forward Automations | ${htmlEscape(LANE_PHRASES[lane] || 'backend and product systems')}</div>`,
...
export async function generateApplicationArtifacts(item, options = {}) {
  ...
  const laneResume = resolveResumeArtifact(effectiveItem, ROOT);
  if (!laneResume.ok) return { ok: false, reason: laneResume.reason };
  const reusedResume = { artifactPath: laneResume.artifactPath, manifest: laneResume.manifest };
  ...
  if (existsSync(output.manifest)) {
    try {
      ...
      const laneResumeCurrent = cached.resumeDecision === 'lane' && cached.resume?.pdfPath === laneResume.artifactPath;
      if (!options.force && laneResumeCurrent && cached.jdHash === effectiveJobHash && cached.sourceHash === currentSourceHash && statusReady && filesReady) {
        const cachedCoverLetterReady = !cached.coverLetter
          || candidateCopyArtifactReadiness(cached, cached.coverLetter.textPath).ok;
        const deliveryResumePdf = options.renderPdf !== false && cached.resume?.pdfPath && existsSync(cached.resume.pdfPath)
          ? copyResumePdfToDelivery(cached.resume.pdfPath, profile)
        ...
        return {
          ...
          coverLetterReady: cachedCoverLetterReady,
          coverLetterCopyQuality: cached.coverLetter?.copyQuality || null,
  ...
  const coverCopyQuality = options.includeCoverLetter === false
    ? null
    : prepareCandidateCopy({ artifactType: 'cover-letter', raw: cover.text }).receipt;
  ...
  const materialChange = previousManifest && (
    ...
    || previousManifest.resumeDecision !== 'lane'
  );
  ...
  const resumeMarkdownPath = '';
  const resumeHtmlPath = '';
  const resumePdfPath = String(reusedResume.artifactPath || '');
  ...
  let resumePdf = resumePdfPath;
  let deliveryResumePdf = '';
  let coverLetterPdf = '';
  let resumeAudit = reusedResumeRecord.audit || { passed: true, errors: [], warnings: [] };
  let coverPageCount = null;
  if (options.renderPdf !== false && options.includeCoverLetter !== false) {
  ...
  if (options.renderPdf !== false && resumePdf && existsSync(resumePdf)) {
    deliveryResumePdf = copyResumePdfToDelivery(resumePdf, profile);
  }
  ...
  const manifest = {
    ...
    coverLetter: options.includeCoverLetter === false ? null : {
      ...
      status: coverCopyQuality?.finalReady === true ? 'ready' : 'draft-humanizer-pending',
      copyQuality: coverCopyQuality,
    },
  };
  ...
  return {
    ...
    coverLetterReady: !manifest.coverLetter
      || candidateCopyArtifactReadiness(manifest, manifest.coverLetter.textPath).ok,
    coverLetterCopyQuality: manifest.coverLetter?.copyQuality || null,
    manifestPath: output.manifest,
  };
}
...
export function inspectArtifactCache(item, options = {}) {
  ...
  try {
    ...
    const coverFilesReady = !manifest.coverLetter
      || (existsSync(manifest.coverLetter.textPath || '')
        && (!manifest.coverLetter.pdfPath || existsSync(manifest.coverLetter.pdfPath)));
    const coverGate = !manifest.coverLetter
      ? { ok: true }
      : candidateCopyArtifactReadiness(manifest, manifest.coverLetter.textPath);
    const coverReady = coverFilesReady && coverGate.ok;
    ...
    return {
      status: fresh ? 'ready' : manifest.coverLetter && !coverGate.ok ? 'humanizer-pending' : 'stale',
      manifestPath: output.manifest,
      lane: manifest.lane,
      selectedProjects: manifest.selectedProjects || [],
      coverLetterReady: coverReady,
    };
  } catch {

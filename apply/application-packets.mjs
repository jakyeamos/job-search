...
import { auditHumanizedText, prepareCandidateCopy, validateCandidateCopyReceipt } from './application-humanizer.mjs';
import {
  applicationAdapter,
...
function answerFromDraft(draft, draftPath = '') {
  ...
  const copy = prepareCandidateCopy({
    artifactType: 'application-answer',
    raw: rawAnswer,
    humanized,
    approvedText: proposedApproval,
    approved: draft.approved === true,
  });
  const approvedAnswer = copy.approved ? copy.output : null;
  return {
    answer: copy.finalReady ? copy.output : rawAnswer,
    rawAnswer,
    humanizedAnswer: copy.humanized,
    approvedAnswer,
    source: 'application-coaching-draft',
    kind: approvedAnswer ? 'approved' : copy.finalReady ? 'humanized' : 'draft',
    evidenceRefs: Array.isArray(draft.evidenceRefs) ? draft.evidenceRefs.map(String) : [],
    humanization: copy.humanization,
    copyQuality: copy.receipt,
...
function coverLetterReview(draft, generatedPath = '') {
  ...
  return {
    ...
    approvedAnswer,
    answer: copy.finalReady ? copy.output : rawAnswer,
    status: approvedAnswer ? 'approved' : copy.finalReady ? 'humanized' : 'draft',
    evidenceRefs: Array.isArray(draft?.evidenceRefs) ? draft.evidenceRefs.map(String) : [],
    humanization: copy.humanization,
    copyQuality: copy.receipt,
  };
...
/** @param {Record<string, unknown> | null} resolved @param {Record<string, unknown>} control */
function ensureNarrativeCopyGate(resolved, control) {
  if (!resolved || !isNarrativeControl(control) || !resolved.answer || resolved.copyQuality || resolved.kind !== 'draft') return resolved;
  const rawAnswer = String(resolved.rawAnswer || resolved.answer);
  const copy = prepareCandidateCopy({
    artifactType: 'application-answer',
    raw: rawAnswer,
    humanized: resolved.humanizedAnswer || '',
    approvedText: resolved.approvedAnswer || '',
    approved: resolved.kind === 'approved' || resolved.approved === true,
  });
  return {
    ...resolved,
    answer: copy.finalReady ? copy.output : rawAnswer,
    rawAnswer,
    humanizedAnswer: copy.humanized,
    approvedAnswer: copy.approved ? copy.output : null,
    kind: copy.approved ? 'approved' : copy.finalReady ? 'humanized' : 'draft',
    humanization: copy.humanization,
    copyQuality: copy.receipt,
  };
}
...
function buildQuestions(item, inspection, profile, ledgerPath, options = {}) {
  ...
  for (const control of controls) {
    ...
    resolved = ensureNarrativeCopyGate(resolved, control);
    const answer = resolved?.answer || null;
    const copyQualityRequired = isNarrativeControl(control)
      && Boolean(resolved?.copyQuality || resolved?.kind === 'draft');
    if (entry && answer && resolved?.evidenceBacked === true
      && (!copyQualityRequired || resolved?.copyQuality?.passed === true)) {
    ...
    const field = {
      ...
      copyQualityRequired,
      copyQuality: resolved?.copyQuality || null,
...
/** @param {Array<Record<string, unknown>>} questions @param {Record<string, unknown> | null} coverLetter */
function candidateCopyGate(questions, coverLetter) {
  const entries = questions
    .filter((question) => question.copyQualityRequired === true)
    .map((question) => ({
      label: String(question.question || 'Application answer'),
      status: String(question.status || 'draft'),
      artifactType: 'application-answer',
      raw: question.rawAnswer || question.answer || '',
      humanized: question.humanizedAnswer || '',
      output: question.approvedAnswer || question.humanizedAnswer || question.answer || '',
      copyQuality: question.copyQuality || null,
    }));
  if (coverLetter) {
    entries.push({
      label: 'Cover letter',
      status: String(coverLetter.status || 'draft'),
      artifactType: 'cover-letter',
      raw: coverLetter.rawAnswer || coverLetter.answer || '',
      humanized: coverLetter.humanizedAnswer || '',
      output: coverLetter.approvedAnswer || coverLetter.humanizedAnswer || coverLetter.answer || '',
      copyQuality: coverLetter.copyQuality || null,
    });
  }
  const evaluated = entries.map((entry) => ({
    ...entry,
    validation: validateCandidateCopyReceipt({
      artifactType: entry.artifactType,
      raw: entry.raw,
      humanized: entry.humanized,
      output: entry.output,
      receipt: entry.copyQuality,
    }),
  }));
  const pending = evaluated.filter((entry) => entry.validation.ok !== true);
  return {
    schema: 'candidate-copy-quality-gate/v1',
    status: pending.length ? 'review_required' : entries.length ? 'passed' : 'not-required',
    finalReady: pending.length === 0,
    requiredCount: entries.length,
    passedCount: entries.length - pending.length,
    pendingCount: pending.length,
    pendingItems: pending.map((entry) => ({
      label: entry.label,
      status: entry.status,
      reason: entry.validation.errors?.[0] || (entry.copyQuality?.status === 'flagged'
        ? 'Humanizer claim audit failed'
        : 'Humanizer revision or receipt is missing'),
    })),
  };
}
...
export function buildPacketMarkdown(packet) {
  ...
  const lines = [
    ...
    `- Candidate copy gate: ${packet.copyQuality?.status || 'not assessed'}`,
    '',
  ];
  ...
  const coverLabel = coverLetter && coverLetter.copyQuality?.finalReady !== true
    ? 'Cover letter (draft — do not attach until Humanizer passes)'
    : 'Cover letter';
  for (const [label, value] of [['Résumé', artifacts.resumePdf], [coverLabel, artifacts.coverLetterPdf || artifacts.coverLetterText]]) {
  ...
  const copyableQuestions = questions.filter((entry) => ['known', 'confirmed', 'evidence-backed', 'approved', 'humanized'].includes(entry.status)
    && entry.answer !== null
    && (entry.copyQualityRequired !== true || entry.copyQuality?.finalReady === true));
  for (const question of copyableQuestions) {
  ...
  const blockedCopyQuestions = questions.filter((entry) => entry.copyQualityRequired === true
    && entry.answer !== null
    && entry.copyQuality?.finalReady !== true);
  if (blockedCopyQuestions.length) {
    lines.push('## Drafts awaiting Humanizer', '');
    for (const question of blockedCopyQuestions) {
      lines.push(`### ${question.question}`, '', `Status: ${question.status || 'draft'}`, 'Do not copy this answer yet. Run the Humanizer stage, then regenerate the packet.');
      if (question.rawAnswer) lines.push('', 'Evidence-bound draft:', '', question.rawAnswer);
      lines.push('');
    }
  }
...
export async function buildApplicationPacket(item, options = {}) {
  ...
  const copyQuality = candidateCopyGate(questions, coverLetter);
  ...
  const packet = {
    ...
    warnings: [
      ...
      ...copyQuality.pendingItems.map((item) => `${item.label}: ${item.reason}`),
    ...
    checklist: [
      ...
      'Copy only candidate-facing text whose copy-quality gate is passed; drafts awaiting Humanizer stay out of the copy/paste section.',
      'Review each prepared nontrivial answer, including humanized narrative text.',

...
| TeamWork Online | HTML | Reads a bounded, host-pinned public category page such as Sports Technology, follows its public pagination, and normalizes job title, company, location, and career level. Configure with `provider: teamworkonline`; optional `exclude_career_levels` prevents source-level Senior/Manager/Intern noise before the normal scanner filters. |

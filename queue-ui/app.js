...
function renderSummary() {
  ...
  const sources = ui.state?.lastRun?.sources || {};
  const labels = { handshake: 'Handshake', wellfound: 'Wellfound', contra: 'Contra', braintrust: 'Braintrust' };
  const lines = Object.entries(labels).map(([id, label]) => {
    const source = sources[id];
    if (!source) return `${label}: not checked`;
    const status = String(source.status || 'unknown').replaceAll('-', ' ');
    const candidates = Number(source.candidates || 0);
    const errors = Number(source.errors || 0);
    const noun = id === 'handshake' ? 'lead' : 'opportunity';
    const suffix = errors ? ` · ${errors} warning${errors === 1 ? '' : 's'}` : '';
    const inbox = id === 'handshake' ? source.inbox : null;
    const inboxSuffix = inbox
      ...
      : '';
    return `${label}: ${status} · ${candidates} cached ${noun}${candidates === 1 ? '' : 's'}${inboxSuffix}${suffix}`;
  });
  elements.sourceHealth.textContent = lines.join(' · ');
}

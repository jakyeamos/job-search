...
function queueItemSummary(item) {
  ...
  return {
    ...
    manifestPath: resolution.manifestPath || null,
...
function plan() {
  ...
  console.log(JSON.stringify({ contractVersion: RESUME_CONTRACT_VERSION, count: selected.length, items: selected }, null, 2));
}
...
function register() {
  ...
  if (!itemId || !artifactPath) {
    throw new Error('Usage: node resume.mjs register --item-id <queue-id> --artifact <configured-lane-pdf>');
  }
  const state = readQueueState(QUEUE_FILE);

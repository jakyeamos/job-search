...
export const RESUME_CONTRACT_VERSION = 2;
...
/** @param {string} root */
function loadResumePolicy(root) {
  const profilePath = join(root, 'config', 'profile.yml');
  if (!existsSync(profilePath)) return {};
  try {
    const parsed = yaml.load(readFileSync(profilePath, 'utf8'));
    return isObject(parsed) && isObject(parsed.cv) ? parsed.cv : {};
  } catch {
    return {};
  }
}
...
function laneArtifactPath(root, lane) {
  const cv = loadResumePolicy(root);
  if (cv.resume_strategy !== 'lane_only') {
    throw new Error('Career Ops requires cv.resume_strategy: lane_only');
  }
  const artifacts = isObject(cv.lane_artifacts) ? cv.lane_artifacts : {};
  const configured = normalizeText(String(artifacts[lane] || ''));
  if (!configured) throw new Error(`no canonical resume artifact is configured for lane: ${lane}`);
  return absolutePath(root, configured);
}
...
export function buildResumeRequest(item, root) {
  ...
  const artifactPath = laneArtifactPath(root, lane);
...
export function resolveResumeArtifact(item, root) {
  ...
  const laneManifest = createResumeManifest(request, artifactPath, {
    root,
    sourceMode: 'lane',
    auditStatus: 'passed',
  });
  return { ok: true, status: 'lane', artifactPath, manifestPath, manifest: laneManifest, request };
}
...
export function registerResumeArtifact(item, root, options) {
  ...
  if (artifactPath !== request.artifactPath) {
    throw new Error(`per-job resume registration is retired; use canonical lane artifact: ${request.artifactPath}`);
  }
  const resolved = resolveResumeArtifact(item, root);
  if (!resolved.ok) throw new Error(resolved.reason);
  return { request, manifest: resolved.manifest, artifactPath, manifestPath: '' };
}

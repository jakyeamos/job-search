 ...
 * never stops pagination, while a page with stale dated postings can stop it.
 * Exported so the repository-wide contract check can pin that behavior.
 */
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    // Liveness probes set a small context cap because they only need proof
    // that the board responds, not a full-board scan. Keep it separate from
    // the configured cap so probe truncation never emits operator advice.
    ...
    // Let the reverse scanner retry transiently truncated tenants later with
    // lower concurrency instead of silently treating partial results as full.

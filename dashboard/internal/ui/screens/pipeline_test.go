...
func TestOpenReportExplainsMissingReport(t *testing.T) {
	...
	if cmd != nil {
		t.Fatal("expected a missing report to block without dispatch")
	}
	if updated.flash != "No report available for this application" {
		t.Fatalf("expected explicit missing-report feedback, got %q", updated.flash)
	}
}

func TestStatusCommandExplainsMissingSelection(t *testing.T) {
	pm := NewPipelineModel(
		theme.NewTheme("catppuccin-mocha"),
		nil,
		model.PipelineMetrics{},
	...
	if cmd != nil {
		t.Fatal("expected an empty pipeline to block without dispatch")
	}
	if updated.statusPicker {
		t.Fatal("expected no status picker without an application")
	}
	if updated.flash != "No application selected to update" {
		t.Fatalf("expected explicit missing-selection feedback, got %q", updated.flash)
	}
}

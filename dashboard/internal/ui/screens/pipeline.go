...
// WithFlash returns the pipeline with a visible one-shot operation result.
// The message replaces the normal help row so keyboard and automation users
// can verify whether a requested mutation committed or failed.
func (m PipelineModel) WithFlash(message string) PipelineModel {
	m.flash = message
	return m
}
...
func (m PipelineModel) handleKey(msg tea.KeyMsg) (PipelineModel, tea.Cmd) {
	...
	case "enter":
		if app, ok := m.CurrentApp(); ok {
			if app.ReportPath == "" {
				m.flash = "No report available for this application"
				return m, nil
			}
	...
	case "c":
		if len(m.filtered) > 0 {
			...
			m.flash = "No application selected to update"
		}

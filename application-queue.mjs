...
/** @param {Record<string, unknown>} item @param {string} adapter @param {{ headless?: boolean, humanHandoff?: boolean, humanTimeoutSeconds?: number, prepareOnly?: boolean, cdpEndpoint?: string }} options @param {{ artifactPath: string }} resume @param {{ coverLetterPdf?: string, coverLetterText?: string, manifestPath?: string, coverLetterReady?: boolean }} [artifacts] */
export function adapterCommand(item, adapter, options, resume, artifacts = {}) {
  ...
  let coverLetterReady = artifacts.coverLetterReady === true;
  if (coverLetterReady && artifacts.manifestPath) {
    try {
      const manifest = JSON.parse(readFileSync(artifacts.manifestPath, 'utf8'));
      coverLetterReady = candidateCopyArtifactReadiness(manifest, artifacts.coverLetterText).ok;
    } catch {
  ...
  if (coverLetterReady && artifacts.coverLetterPdf) command.push('--cover', artifacts.coverLetterPdf);
  if (coverLetterReady && artifacts.coverLetterText && existsFile(artifacts.coverLetterText)) {
...
export async function prepareApplicationArtifacts(item, policy) {
  ...
  item.coverLetterArtifact = artifacts.coverLetterReady === true ? artifacts.coverLetterPdf || null : null;
  item.coverLetterText = artifacts.coverLetterReady === true ? artifacts.coverLetterText || null : null;
  item.coverLetterStatus = artifacts.coverLetterReady === true ? 'humanizer-passed' : 'draft-humanizer-pending';
  item.coverLetterCopyQuality = artifacts.coverLetterCopyQuality || null;
...
export async function runApplicationQueue(options = { dryRun: false, limit: 6, headed: false }) {
  ...
  for (const item of items.slice(0, dailyCap)) {
    ...
    const rawResult = await runAdapter(adapterCommand(item, adapter, { headless: !options.headed }, resume, {
      ...
      manifestPath: item.applicationArtifactManifest,
      coverLetterReady: item.coverLetterArtifact || item.coverLetterText ? item.coverLetterStatus === 'humanizer-passed' : false,
    }), 180_000)

...
## Candidate-copy finalization gate

All prose the candidate may send or publish must pass through the final
Humanizer stage before it is labeled final. If the draft needed research,
complete RDW Domain QA first, then run RDW's Humanizer/blader stage. For
style-only work, run Humanizer directly.

The final copy must preserve concrete claims from the evidence-bound draft.
When the executable application path is used, it must carry a content-bound
`candidate-copy-quality-receipt/v1` whose input and output hashes still match
the exact text and whose Humanizer status is `passed`. A missing receipt,
changed text, or failed claim audit means `Draft — Humanizer pending`; keep it
out of copy/paste or attachment-ready output. Human review and the
human-only submission boundary still apply after Humanizer passes.
The application artifact and queue boundary also verifies the exact cover-letter
text file before an ATS adapter receives it. Em and en dashes are currently
rejected by that executable style audit and must be replaced before finalization.

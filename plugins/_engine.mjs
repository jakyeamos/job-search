 ...
 * @param {{ root: string, dryRun?: boolean, timeoutMs?: number, onlyIds?: string[] }} opts
 * @returns {Promise<Array<{ id: string, ok: boolean, result?: any, error?: string }>>}
 */
export async function runHook(kind, payload, { root, dryRun = false, timeoutMs = DEFAULT_HOOK_TIMEOUT_MS, onlyIds = [] }) {
  await loadDotenvOnce();
  const loaded = await loadPlugins(kind, { root, dryRun });
  const selected = onlyIds.length ? loaded.filter(({ id }) => onlyIds.includes(id)) : loaded;
  const results = [];
  for (const { id, hook, ctx } of selected) {

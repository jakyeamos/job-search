...
test('compact triage brief mirrors the graduated experience-floor policy', () => {
  const brief = readFileSync(new URL('../modes/_brief.md', import.meta.url), 'utf8');
  assert.doesNotMatch(brief, /Minimum experience stated is 3\+ years/i);
  assert.match(brief, /3 years: mild downscore, never a blocker/i);
  assert.match(brief, /4-5 years: heavy downscore; block only when the role's substance also misses/i);
  assert.match(brief, /6\+ years: hard DQ/i);
});
